...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.workshopResponsible.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.workshop-responsibles.store") }}" enctype="multipart/form-data">
            @csrf
         ...
         <div class="form-group">
                <label class="required" for="visisble_password">{{ trans('cruds.workshopResponsible.fields.visisble_password') }}</label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.workshopResponsible.fields.visisble_password_helper') }}</span>
                </div>
            <div class="form-group">
                <label class="required" for="bank_code">{{ trans('cruds.workshopResponsible.fields.bank_code') }}</label>
                <input class="form-control {{ $errors->has('bank_code') ? 'is-invalid' : '' }}" type="number" name="bank_code" id="bank_code" value="{{ old('bank_code', '') }}" step="1" required>
